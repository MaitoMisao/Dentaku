...
class CalcEngine {
    ...
    var register: String = "0" // ユーザの入力と＝ボタンで確定した計算結果を保存
    var _result: String = "0" // これまでの計算結果を保存
    var mode :Mode = .none
    ...
    func number(input:Int){ //
        ...
        var registerVal = Decimal(string:register)! // 変換に失敗する場合は今回は考慮しない
    ...
    func equal(){
        switch mode {
        case .plus:
            let resultVal = Decimal(string:_result)!
            result = "\(resultVal + Decimal(string:register)!)"
            register = "0"
            mode = .none
